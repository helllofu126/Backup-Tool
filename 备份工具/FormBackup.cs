...
namespace HelloFu.BackupTool
{
    ...
    public partial class FormBackup : Form
    {
        ...
        private async void ZipFile(List<string> sourceFilePathList, List<string> destinationZipFilePathList, string backupName)
        {
            ...
            {
                //拼接ZIP文件的路径
                var zipFilePathTemp = directoryPath + "\\" + backupName;

                //获取ZIP文件的路径
                var zipFilePath = Path.GetFullPath(zipFilePathTemp);

                //获取ZIP文件的目录路径
                var zipDirectoryPath = Path.GetDirectoryName(zipFilePath);
                ...
                {
                    var result = MessageBox.Show($"文件{zipFilePath}已存在，是否覆盖？", "文件已存在", MessageBoxButtons.YesNo);
